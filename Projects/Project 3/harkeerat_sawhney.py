...
class TextDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Return the sequence
        return sequence[:-1].clone().detach().to(device), sequence[1:].clone().detach().to(device)
        
        
# Implementing the LSTM model
class LSTMModel(nn.Module):
    def __init__(self, word_to_int, vocab_size, embedding_dim, hidden_dim, output_dim, n_layers, drop_prob=0.5):
...
def sample(model, sampler_function, word_to_int, int_to_word, device, prompt):
    ...
    while True:
        ...
        prompt_tokens = torch.cat([prompt_tokens, torch.tensor([[next_word_index]]).to(device)], dim=1)
...
def training_loop(model, dataloader, loss_function, optimizer, device, word_to_int, int_to_word, epochs, k, clip=None):
    model.train()
    ...
    while epoch < epochs:
        ...
        if epoch == 1 or epoch == epochs/2 or epoch == epochs:
            # Generate a sample
            generated_sentence = sample(model, sampler_argmax, word_to_int, int_to_word, device, "earth")
    ...
    while epoch < max_epochs:
        ...
        for input, output in dataloader:
            ...
            for j in range(n_chunks):
                ...
                # Calculate loss
                loss = criterion(output.view(-1, model.vocab_size), output_chunk.view(-1))
        ...
        print(f'Epoch: {epoch}, Loss: {average_loss}, Perplexity: {np.exp(average_loss)}', f'Time: {time.strftime("%H:%M:%S", time.localtime())}')
...
if __name__ == "__main__":
    ...
    df = pd.read_json('/kaggle/input/newscategory/News_Category_Dataset_v3.json', lines=True)
    ...
    if os.path.exists('./data/word_to_int.pkl') and os.path.exists('./data/int_to_word.pkl') and os.path.exists('./data/all_words.pkl'):
    ...
    if batch_size == 1:
        ...
        dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True, collate_fn=lambda x: collate_fn(x, word_to_int['<PAD>']))
    ...
    model = LSTMModel(word_to_int, vocab_size, embedding_dim, hidden_dim, output_dim, n_layers, drop_prob)
    ...
    model, losses, perplexities = training_loop(model, dataloader, loss_function, optimizer, device, word_to_int, int_to_word, epochs, 2, clip)
    ...
    model_new = LSTMModel(word_to_int, vocab_size, embedding_dim, hidden_dim, output_dim, n_layers, drop_prob)
    model_new.to(device)
    
    optimizer = optim.Adam(model_new.parameters(), lr=lr)
    loss_function = nn.CrossEntropyLoss(ignore_index=word_to_int['<PAD>'])
    
    model_new, losses, perplexities = train_with_TBBTT(epoch, model_new, dataloader, loss_function, optimizer, chunk_size, device, clip)
    
    torch.save(model_new.state_dict(), '/model/TBBTT_model.pt')

    # Import the model
    original_model = LSTMModel(word_to_int, vocab_size, embedding_dim, hidden_dim, output_dim, n_layers, drop_prob)
    ...
    new_model = LSTMModel(word_to_int, vocab_size, embedding_dim, hidden_dim, output_dim, n_layers, drop_prob)
    new_model.to(device)
    ...
    for i in range(3):
        generated_sentence_original_mode = sample(original_model, random_sampler_next, word_to_int, int_to_word, device, "the president wants")
        print("Generated Sentence for Sampling Strategy with Original Model {}: {}".format(i+1, generated_sentence_original_mode))
        
        gererated_sentence_new_model = sample(new_model, random_sampler_next, word_to_int, int_to_word, device, "the president wants")
        print("Generated Sentence for Sampling Strategy with New Model {}: {}".format(i+1, gererated_sentence_new_model))
        
        generated_sentence_original_mode_greedy = sample(original_model, sampler_argmax, word_to_int, int_to_word, device, "the president wants")
        print("Generated Sentence for Greedy Search with Original Model {}: {}".format(i+1, generated_sentence_original_mode_greedy))
        
        gererated_sentence_new_model_greedy = sample(new_model, sampler_argmax, word_to_int, int_to_word, device, "the president wants")
        print("Generated Sentence for Greedy Search with New Model {}: {}".format(i+1, gererated_sentence_new_model_greedy))
